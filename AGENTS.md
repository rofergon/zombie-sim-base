...
voice lines), procedural dynamic score and file-safe CC0 ambience; unlocks on
first pointerdown and the scenario names events plus soundscape/score state
js/main.js bootstrap: world, camera, input, main loop

`zone.html` additionally loads
`js/zone/{config,geo,state,weapons,map,logistics,vehicles,threats,regions,orders,citizens,tasks,gathering,squads,scavenge,adaptations,agriculture,fortifications,defense,campaign,ui}.js` and
...
| `ZS.sound`                                              | `event(name, x, y)`, `tick(dt)`, `unlocked` — synthesized WebAudio cues plus file-safe CC0 ambience, camera spatialization, per-name cooldowns and a procedural score selected by `soundscape(scene)`; unlocked on first pointerdown. Event names are owned by the scenario (zombie map: `shot_rifle/shot_shotgun/shot_smg/shot_gren/boom/moan/door_break/fire/turret/horn` + the formant voice lines `v_shout/v_gasp/v_mumble/v_laugh/v_grunt/v_callout/v_groan/v_growl/v_chomp/v_mama/v_spit/v_zedshout` — moving F1/F2/F3 bandpasses over a detuned double-saw glottis, ported 1:1 from `.verify/voices.html`; `moan` picks a random dark zombie line; `boom` is a full phrase: soft-clipped shock crack, falling bandpass body, saw sub, resonant rumble tail, debris, distant echo). Zone calls `scene.layer`, `scene.emitter` and `scene.score`; its score cues cover wilderness, day phases, warnings, combat and outcomes. |
...
| `soundscape(scene)`                                   | optional: low-frequency camera audio description; use `scene.layer(index, key, gain)` and `scene.emitter(id, key, x, y, gain, range)` for ambience, plus `scene.score(cue, intensity)` for the procedural music director                                                                                                                                                                                                                                                                                                                                                          |
